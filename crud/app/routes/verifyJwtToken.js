...
var verifyToken = function (req, res, next) {
  ...
  if(!token) {
    return res.status(403).send({ auth : false, message : "No token provided (< x-access-token > header empty)" });
  }

  jwt.verify(token, config.secret, function(err, decoded) {
    if(err) {
      return res.status(500).send({ auth : false, message : "Fail to Authentication. Error -> " + err });
    }
